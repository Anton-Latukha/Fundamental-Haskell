...
GitHub & GitLab parse Org into HTML only partially. Good quality HTML export is in the =README.html=.
...
When you can not formulate the code, or do not want to folmulate with your constrain on the join of parts - it is simple and useful to write only in a needed constrain manner first. At some point of that you would need to start pick and satisfy constraints, you can do it one-by-one at a time.

...
Laziness allows to skip some issues, parts that are/were not used - never throw exceptions, but also exceptions are raised in functions at different times on computations, during the laziness process.

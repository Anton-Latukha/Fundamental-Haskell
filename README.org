...
Category $$ \mathcal{C} $$ comonad is a monad of oposite category $$ \mathcal{C}^{op} $$.
...
In words, /*/ depends on $$ F $$ and $$ G $$ functors, and:
...
$$ \overrightarrow{\eta}^{\mathcal{D}}(x) = F^{\mathcal{D}}(x) \to G^{D}(x) \ | \  x \in \mathcal{C} $$
...
Configuration was reassembled from: https://github.com/emacs-lsp/lsp-haskell/blob/8f2dbb6e827b1adce6360c56f795f29ecff1d7f6/lsp-haskell.el#L57 authors config: https://github.com/sevanspowell/dotfiles/blob/master/.spacemacs

Refresh Emasc.

*** 4. Open the Haskell file from a project

Observe in system monitor the proces of package loading & compilation.

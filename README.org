#+title: Fundamental Haskell
#+author: Anton Latukha
#+creator: Self-published by Anton Latukha
#+description: Notes on Haskell, Category theory and related fields.
#+subtitle: Encyclopedic handbook for learning and understanding fundamentals
#
#
#
...
:PROPERTIES:
:ID:       5e228850-1b4f-49a7-bc88-faf6f518885d
:END:
  ...
  - [[#data-type][<<<Data type>>>]]
    ...
    - [[#type-class][<<<Type class>>>]]
      ...
      - [[#derived-instance][<<<Derived instance>>>]]
        - [[#-70][/*/]]
      - [[#type-class-functional-dependency][<<<Type class functional dependency>>>]]
...
- [[#good-code][Good code]]
  ...
  - [[#good-forget-error--use-fail-instead][Good: Forget ~error~ , use ~fail~ instead]]
- [[#bad-code][Bad code]]
...
- [[#draft][Draft]]
  ...
  - [[#algorithm-of-the-hackage-package-release][Algorithm of the Hackage package release]]
    ...
    - [[#open-up-git-diff-lastverhead-on-one-side-of-the-screen][Open-up ~git diff <lastVer>..HEAD~ on one side of the screen]]
    ...
    - [[#add-a-git-tag-v][Add a ~git tag <v>~]]
  ...
  - [[#fixed-point][<<<Fixed point>>>]]
    - [[#attractive-fixed-point][<<<Attractive fixed point>>>]]
    - [[#least-fixed-point][<<<Least fixed point>>>]]
      - [[#fix][<<<Fix>>>]]
  - [[#-210][]]
  - [[#conways-law][<<<Conway's law>>>]]
  - [[#f-algebra][<<<F-algebra>>>]]
  - [[#cabal-artifacts][Cabal artifacts]]
  - [[#hpack][~Hpack~]]
- [[#reference][Reference]]
  - [[#history][History]]
  ...
  - [[#haskell-package-versioning-policy][<<<Haskell Package Versioning Policy>>>]]
    - [[#-212][/*/]]
...
4) In Haskell mosr often used as substitute to "particular implementation" on top of some (considered well-known or basis to the code by the code author) theory/principle/abstraction mechanism.
...
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:09]
:end:
...
Translating objects and morphisms (so as an input can take morphism or object of the source category).

"Full translation" means every part gets represented in some shape or form, but at all does not means ideal translation. For example, all categories can be translated into (mapped onto) a single object monoidal category.
/*/ - forgetful - discards part of the structure.
/*/ - faithful - gives unique isomorphic (bijective) preservation of all source morphisms (injective on Hom-sets).
/*/ - full - translation of morphisms fully covers all the morphisms between according objecs in the target categoty (functor analog of epimorphism).

For Haskell Functor type class or ~fmap~ - see Power set functor.

Definition (axiomatic properties):
=Functor=:
...
:DRILL_EASE: 2.18
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2020-08-04 Tue 19:31]
:end:
...
Functor type class in Haskell defines a /*/ as ~fmap~ in a form of bifunctor. It allows to do function application inside type structure layers (denoted \( f \) or \( m \)). IO is also such structure.
Power set is unique to the set, /*/ is unique to the data type (to the category also).
Any endofunctor embodies into /*/. It is easily seen from Haskell definition - that the /*/ is the polymorphic generalization over any endofunctor in a category.
...
Composition of /*/ always produces /*/. (/*/ are closed unders composition).
...
To compose functions contained within applicatives is the same as using normal function composition inside of an applicative, aka function composition works regularly.
#+begin_src haskell
pure (.) <*> u <*> v <*> w = u <*> (v <*> w)
#+end_src
...
pure (.) <*> Right (+20) <*> Right (+10) ==
(Right <> Right) ((.) (+20) (+10)) ==
(Right <> Right) (+20 . +10) ==
Right (+30)

pure (.) <*> Right (+20) <*> Right (+10) <*> Right 5 ==
Right ((.) (+20) (+10) (5)) ==
Right ((+20 . +10) (5)) ==
Right ((+30) 5) ==
Right 35
...
& also:
#+begin_src haskell
Right (+20) <*> (Right (+10) <*> Right (5)) ==
Right (+20) <*> ((Right <> Right) (+10) (5)) ==
(Right <> Right <> Right) ((+20) (15)) ==
Right 35
#+end_src

Aka, ~{<*>, pure}~ represent ~{<>, ' ' (function application)}~ at the same time. How applicative operators applied inside of the Haskell expression - provides information how to compose the applicative structure as a monoid, and contents as a function application. Rules they apply by are just as they would be if to remove applicative operators and ~{applicative structures OR context}~ (depenting on what to stay).

#+begin_src haskell
pure (.) <*> au <*> bv <*> cw = au <*> (bv <*> cw)
-- Contents applies:
(.) u v w = u (v w)
pure (.) <*> au <*> bv <*> cw = au <*> (bv <*> cw)
-- Applicative structure applies:
pure <> a <> b <> c = a <> (b <> c)
#+end_src
...
This shows that because ~<*>~ applies function to the argument - the order is important.
In other words - it is not commutative. And to turn the application order in ~<*>~ the argument needs to be demoted (~$~).
...
pure ($ y) <*> uf   -- `$` is infix operator, so it binds `y` as second argument: `(\ f y -> f $ y)`
pure (($) y) <*> uf
(pure <> u) (f $ y)
#+end_src
...
:DRILL_EASE: 2.42
:DRILL_LAST_QUALITY: 4
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:07]
:end:
...
Contrary to applicative functors composition of which always produces applicative, composition of monads does not always results in monad. (Monads are not closed under composition).
...
:DRILL_EASE: 2.14
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:24]
:end:
...
#+begin_src haskell
<$>            :: Monad f => (a -> f b) -> f a -> f (f b)
join           :: Monad f =>                      f (f a) -> f a
join $ f <$> a :: Monad f => (a -> f b) -> f a            -> f b
=<<            :: Monad f => (a -> f b) -> f a            -> f b
#+end_src
...
Sequencing operator, analog to the semicolon (or newline) in imperative languages.
Discard value result of the action, sequentially add next (compose with) next action.
...
:DRILL_EASE: 2.18
:DRILL_LAST_QUALITY: 4
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:06]
:end:
...
:DRILL_EASE: 2.28
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:08]
:end:
...
:DRILL_EASE: 2.1
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:19]
:end:
...
:DRILL_EASE: 2.04
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:15]
:end:
...
:DRILL_EASE: 2.6
:DRILL_LAST_QUALITY: 4
:DRILL_LAST_REVIEWED: [2020-08-07 Fri 10:47]
:END:

Enumerated (ordered) set.
Set with a strict order property.
...
:DRILL_EASE: 2.46
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:25]
:end:
...
Type classes with multiple parameters are relations on types.
...
**** <<<Type class functional dependency>>> :drill:

/*/ declares a promise of a relation that from one type would uniquly determine/determines another.
...
/*/ are used for multi-parameter type classes. As some multi-parameter type classes with their methods create undecidable situations when method signature & use can not infer all parameters for instance of the type class. Solution in the case when one of the parameters can be determined from the others - /*/ get declared, so (subordinate parameter) then for example can be the return type but none of the argument types of some of the methods, which allows type system to work propely.
...
There is only one way to "perform" an I/O action: bind it to ~Main.main~ of the program.
For function I/O it should be in IO monad and evaluated from ~Main.main~.
...
:DRILL_EASE: 2.24
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:17]
:end:
...
It is a property of \(\{f^{\mathcal{X \to X}}, \mathcal{X}\}\) (function & a set it operates on).
...
It is a powerful property. For example:
 1) it allows to apply algebra operations (\(\{*,/,+,-\}\)) without "does operation fits" to any numbers in the numbers in the \(\mathbb{R}\), because those operations are closed under the \(\mathbb{R}\), with a caveat of /small/ caveat of \(0\) for \(/\).
 2) it allows to apply and compose endofunctors on categories without thought or remorse of whenever that would work, since any combination of them works. Because ~fmap~ defined for all ~[]~ or for ~Maybe~ and does not changes structure. For example one can apply ~fmap~ to ~[Maybe a]~ to a list, or compose ~fmap~ of `Maybe` with ~fmap~ of ~[]~ (in Haskell just compoe twice ~fmap . fmap~) to work on the ~Maybe a~ values inside ~[]~. Again, because Haskell functors are endofunctors defined on the whole type which is polymorphic inside, so the definitions of Haskell endofunctors are closed in types they apply to.
...
Classical type system for lambda calculus with parametric polymorphism and type inference. Types marked as polymorphic variables, which enables type inference over the code.
...
:DRILL_EASE: 2.46
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:20]
:end:
...
:DRILL_EASE: 2.38
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:27]
:end:
...
:DRILL_EASE: 2.6
:DRILL_LAST_QUALITY: 4
:DRILL_LAST_REVIEWED: [2020-08-07 Fri 10:49]
:END:
...
In a classical general sence of /*/ (noncommutative associativity) - operation assignes/gives some (hidden) additional meaning to the operands order (positions), and their positioning also influence the result. In commutative case the result depends only on information of the operands.
...
:DRILL_EASE: 2.6
:DRILL_LAST_QUALITY: 4
:DRILL_LAST_REVIEWED: [2020-08-07 Fri 10:35]
:end:
...
:DRILL_LAST_REVIEWED: [2020-08-07 Fri 10:36]
:end:
...
:DRILL_EASE: 2.42
:DRILL_LAST_QUALITY: 4
:DRILL_LAST_REVIEWED: [2020-08-05 Wed 12:12]
:end:
...
Multiparameter generalizes type classes with interpretation of them being relations on types.
...
****** <<<TypeOperators>>> :drill:
:PROPERTIES:
:ID:       2e1352a2-53d9-4c4d-b5d4-473353d0a87c
:END:
...
** Good: Forget ~error~ , use ~fail~ instead
...
Automatically translates Cabal/Stack project and dependencies into Nix code. Provides IFD (import from derivation) functions that minimize the amount of Nix code that is needed to be added.
...
Shortly describe where is speccing happens & belongs, where is parsing happens & belongs, where one can custom hande parsed data on top of what is provided in lib. So now readers roughly know the data model and what are structural parts and where they are
...
#+end_src
  ...
  * ~argument :: ReadM a -> Mod ArgumentFields a -> Parser a~
    General implemetation that uses given reader to parse direct argument.
  * ~strArgument :: IsString s => Mod ArgumentFields s -> Parser s~
    To consume a string argument directly.
  * ~option :: ReadM a -> Mod OptionFields a -> Parser a~
    General implementation. Allows to use the given reader.
  * ~flag :: a {default value} -> a {active value} -> Mod FlagFields a {option modifier} -> Parser a~
  ...
  * ~switch :: Mod FlagFields Bool -> Parser Bool~
    ...
    #+end_src
    Irrefutable \Rightarrow no termination for ~some~ or ~many~, for them use ~flag'~
  * ~flag' :: a {active value} -> Mod FlagFields a	{option modifier} -> Parser a~
    Flag parser without a default value. Has sence in composite parser, or when requiring ~--on~ OR ~--off~ alternatives.
  * ~infoOption :: String -> Mod OptionFields (a -> a) -> Parser (a -> a)~
    Always stops binary and displays a message.
...
*** Parsers
    ...
    How parsers are composed from attributes and builders
...
*** Create git branch ~release x.x.x.x+1~
*** Open-up ~git diff <lastVer>..HEAD~ on one side of the screen
*** Open ~CHANGELOG.md~ on the other side of the screen
*** Walk through diff and populate ~CHANGELOG.md~
...
***** (optional) API additions of functionality
***** (optional) Other changes in the project, news

*** Check ~cabal sdist~ build passes
*** Think what new files can/should be included in ~.cabal~ ~extra-source-files~
*** Update ~.cabal~ ~version:~
*** Add a ~git tag <v>~
*** ~git push --tags~
*** (optional) (Remove git tag)
...
*** Base IO Data types
:PROPERTIES:
:ID:       67f238bf-c61b-4894-a859-21e8d95e26c2
:END:
...
Essential properties:
  ...
  * Status: open, semi-closed, closed.
    A good practice to close handlers with ~hClose~, aslo garbage collector at some point most probaly would remove unused handlers.
  * Seekable: ~True | False~.
...
Since Haskell does not allow infinite types, fixed point can not be expressed in the type signature, so the recoed type is used instead.
A type is isomorphic to the recursively approaching the attactive fixed point process of the least fixed point 
#+begin_src haskell
...
| ~packagedb/~      | project build ~cabal metadata~ package database.                                                          |
| ~cache/~          | build plan package constraints solver artifacts. Hashes of external packages.                           |
| ~*.hi~            | interface modules: type information, public exports, symbol table, other internals of compiled modules. |
| ~*.hie~           | interface extended file: module symbol data.                                                            |
| ~*.hspp~          | preprocessor.                                                                                           |
| ~cabal_macros.h~  | CPP-accessible preprocessor definitions.                                                                |
| ~*.o~             | Compiled object files per module.                                                                       |
| ~*.ll~            | LLVM source file.                                                                                       |
| ~*.bc~            | LLVM bytecode.                                                                                          |
| ~*.s~             | Assembly.                                                                                               |
...
Cabal file generator YAML-format DSL that aims to reduce repetitions.

DHall excels at doing the same.
...
*** [[http://www.datahaskell.org/docs][data Haskell]] - (2017) annotated links to data science & machine learning libraries, overviews and benchmarks of libraries

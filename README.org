...
Functor translates morphisms and their composition deterministicly (referentially transparent). So there is no difference:
  a) or translate everything one-by-one and assemble at destination category.
  b) or assemble everything in source cetegory and translate in one go once.

Composing and translating at once optimizes computation (see fusion).

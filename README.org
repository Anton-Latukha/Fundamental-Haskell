...
You probably would hear saying: =Haskell has immutable "variables"= (term used in imperative programming sence). Logically we see statement nonsensical in meaning, "variables" (something that has change as a defining propery) are not changing, aka, nonsence. It is due to programmers tend to speak in terms of imperative programming. Read the saying as: =Haskell has immutable values=. In haskell values (due to value semantics (see "Value")) are immutable, and Haskell expressions are functions (that are referentially transparent - meaning itself immutable) are also, and since they bind to /*/, people say they symbols (according "/*")/ are immutable (inside scope area immutable value is binded in). (Except, when you hack Haskell with explicit low-level funсtions, and start to directly mute values.)

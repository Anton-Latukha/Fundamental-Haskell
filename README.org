...
- [[#drafts][Drafts]]
  ...
  - [[#function-object-exponential][Function object, exponential]]
...
\begin{document}
\begin{tikzcd}
a \arrow[rrrr, "F" description, bend left] \arrow[rrrr, "G" description, bend right] \arrow[rrrr, "H" description, bend right=49] &  & \Downarrow{\alpha} &  & b \\
                                                                                                                                  &  & \Downarrow{\beta}  &  &  
\end{tikzcd}
\end{document}
...
:DRILL_EASE: 2.36
:DRILL_LAST_QUALITY: 4
:DRILL_LAST_REVIEWED: [2020-02-20 Thu 22:39]
:end:
...
:DRILL_LAST_INTERVAL: 8.4538
:DRILL_REPEATS_SINCE_FAIL: 4
:DRILL_TOTAL_REPEATS: 27
:DRILL_FAILURE_COUNT: 15
:DRILL_AVERAGE_QUALITY: 2.593
:DRILL_EASE: 1.34
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2020-02-20 Thu 22:41]
:end:
...
:DRILL_EASE: 2.36
:DRILL_LAST_QUALITY: 4
:DRILL_LAST_REVIEWED: [2020-02-20 Thu 22:38]
:end:
...
Category where:
  * object (0-cell) - category
  * \Rightarrow morphism (1-cell) - functors
  * \Rightarrow 2-morphisms (2-cell) - natural transformations, homotopy between morphisms.
...
Has Hom objects, gets away Hom sets.

$$ hom(x,y) \in K $$.

As category is no longer small, may be called large.

Hom objects are includedd in some monoidal category =K= , because multiplication operation allows composition of Hom objects: 

$$ \circ:hom(y,z) \otimes hom(x,y) \to hom(x,z) $$

So, it always the case that category is indicates as /*/ over =K=.
...
Enriched lux 2-category.

Category that escaped the Set theory and the equality definition (by using associativity and idintity "up to isomorphism" (and introducing associator, left/right unitor)).

But, paradoxically, forming from bicategories higher categories/stacking levels of such categories - leads to special cases, differences of every level and difficulties.

Stacking groupoids is much more homogenous up to infinity, and forms base of the homotopy theory.

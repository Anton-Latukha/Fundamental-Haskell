...
If module has any orphans - in GHC terms all module is called "orphan module". GHC "visits the interface file of every orphan module below the module being compiled. This is usually wasted work, but there is no avoiding it. You shouldtherefore do your best to have as few orphan modules as possible" ([[https://downloads.haskell.org/~ghc/latest/docs/users_guide.pdf]["GHC User’s Guide Documentation, Release 8.8.3"]]). So orphan prolongs the compilation, and moreover - compilation of all dependent code, because requires recursive lookups into module dependencies.
...
:DRILL_EASE: 2.6
:DRILL_LAST_QUALITY: 5
:DRILL_LAST_REVIEWED: [2020-03-28 Sat 19:13]
:END:
...
\( \forall \) Universal quantifier - a general solution.
\( \exists \) Existential quantifier - a local solution.

\( \forall \) and \( \exists \) are tao-dual:
\( \forall A_{}(x) \ : \ B \ \iff \exists B(x) \ : \ A \)

In Haskell with RankNTypes types that have:
\forall before \Rightarrow universal
\forall after \Rightarrow existential

Haskell existentials are always result in throwing aways type information.

Useful to be able to work with data from at external world that we do not know definite type at compile time.

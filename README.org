...
/*/ - Monoid in Endofunctor category (endofunctor) with unit ant join natural transformations.
...
Mostly Monads used for sequencing actions (that looks like imperative programming), with dependency on previous chains of the sequence, there are Commutative Monads that do not order actions.

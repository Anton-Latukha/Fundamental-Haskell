...
Ways to pass arguments from function to function:
  ...
  * Lazy (outside-in) evaluation - passing an argument by-name. Argument expressions don't evaluate at call time. When on execution of the function body a reference to the parameter is encountered - a coresponding expressions begins evaluation. Immutability allows to share the argument directly into the function.

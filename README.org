...
Working with code from the point of constraints. An axiomatic semantics approach.

+ It should be used when you don't want or can not formulate the join points in code for types.
- With this approach you can not formulate the calculation processes
- It is tedious (a lot of boilerplate and rewriting boilerplate into the result) but pretty relaxing
+ Useful to write only in a needed constraints/types first. At some point of that you would need to start pick and satisfy constraints, you can do it one-by-one at a time.
...
*Are mixing the transformers produce the same result?*

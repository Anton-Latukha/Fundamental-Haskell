...
Properties (or axioms):
  ...
  2. All objects have identity morphisms: $$ 1_x \circ f_{a \to x} \equiv f_{a \to x}, \; g_{x \to b} \circ 1_x \equiv g_{x \to b }\ | \ \forall x \ \exists 1_{x \to x}, \forall f_{a \to x},  \forall g_{x \to b} $$ (two-sided).

...
class Functor f where
  fmap :: (a -> b) -> f a -> f b
#+end_src
Functor in Haskell is a type class presents of which allows function application "over/through" type structure layers (denoted /f/ of /m/). IO is also such structure.

Haskell Functor Type class corresponds to mathematical Powerset functor.
Powerset functor is unique to the category (data type).

...
Homomorphism - operation-preserving map between two algebraic structures of the same type.

$$  f^{A \to B}_{homomorphism} = f(x \star y) = f(x) \diamond f(y) | (A_{set}, \star_{operation}), (B_{set}, \diamond_{operation}) $$.

Homomorphism sends identity morphisms to identity morphisms and inverses to inverses.

...
=SCHT:= - for my =org-drill= practice. It also stores metadata in :PROPERTIES:. Please just run =org-drill-strip-all-data= to remove my metadata and =SCHT:=.
...
- [[#definitions][Definitions]]
  ...
  - [[#natural-transformation][<<<Natural transformation>>>]]
    - [[#natural-transformation-component][Natural transformation component]]
      - [[#-40][/*/]]
        - [[#component-of-natural-transformation][Component of natural transformation]]
...
:DRILL_EASE: 2.5
:DRILL_LAST_QUALITY: 4
:DRILL_LAST_REVIEWED: [2019-06-06 Thu 16:07]
:END:
...
Multiplies [[Cardinality][cardinalities]].
...
/*/ - [[Monoid]] in [[Endofunctor category]] ([[Endofunctor][endofuntor]]) with both natural transformations.
...
Homomorphism - operation-preserving map between two [[Algebraic][algebraic]] structures of the same type (groups, rings, vector spaces...).
...
** <<<Phrase>>> :drill:
/*/ - [[Composition][composable]] expression.
** <<<Proxy type>>> :drill:
:PROPERTIES:
...
/*/ [[Function][functions]] are [[Pure][pure]].
** <<<Relation>>> :drill:
:PROPERTIES:
...
/*/ structure-preserving transformation of one [[Functor]] into another.
/*/ is a "morphism of functors", especially in functor categories.
/*/ by a certain rule for every object picks [[Natural transformation component][natural transformation component]]: $$ \forall o \in \mathcal{C} : hom^{\mathcal{C \to D}}(F^{\mathcal{C \to D}}(o) \to G^{\mathcal{C \to D}}(o)) $$

Bartosz lecture: https://www.youtube.com/watch?v=2LJC-XD5Ffo
Category defines structure.
Functors are preserving structure mappings.

There are several ways (functors) for modelling one category inside other category.
Natural transformation - mapping between two functors. Natural transformation also preserves structure.

$$ (\mathit{cat} \ \mathcal{C, D}), (\mathit{functor} F^{\mathcal {C \to D}}, G^{\mathcal{C \to D}}) $$

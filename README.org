...
This has an effect that on stacking monad transformers, =m= becomes monad stack, and every next transformer injects the transformer creation-specific properies $$ \eta $$ inside the stack, so out-most transformer has inner-most structure.
...
To create a Reader monad from /*/: $$ m \equiv Identity $$:
#+begin_src haskell

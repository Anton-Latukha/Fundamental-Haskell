...
- [[#reference][Reference]]
  ...
  - [[#mathematical-symbols-dictionary][Mathematical symbols dictionary]]
- [[#liturgy][Liturgy]]
...
| $$ \overrightarrow{\eta}_{ID \to E} $$       | = pure ∷ Applicative f ⇒ a → f a =        | unit (natural transformation for functors $$ ID \to \mathcal{P} $$) |
| $$ \overrightarrow{\mu}_{E \circ E \to E} $$ | = join ∷ Monad f ⇒ f (f a) → f a =        | join (natural transformation for functors $$ \mathcal{P \circ P \to P} $$) |

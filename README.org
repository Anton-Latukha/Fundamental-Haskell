...
#+latex_header:     \unimathsetup{math-style=TeX}    % Configures the unicode-math
# #+latex_header:     \setmainfont[Ligatures=TeX]{Constantia}
# #+latex_header:     \setsansfont{Calibri}
# #+latex_header:     \setmonofont{Consolas}
# #+latex_header:     \setmathrm{Cambria Math}
# #+latex_header:     \setmathfont{Cambria Math}
#+latex_header: \else
...
#+latex_header:     \usepackage[utf8x]{inputenc} % input encoding
#+latex_header: \fi
...
As you see in the code - Haskell variables (same names) hold different value at different time. Variables are reused, meaning "names are reused, binded to different values on scope changes/cycles, and can be shadowed. But all values that Haskell holds - are, by the design of the language, are treated immutable, any transformations Haskell resolves by creating new values, and frees the space by freeing-up from no longer needed values.

...
Primitives:
  ...
  3. Morphism composition - binary operation: $$ C(a, b) \circ C(b, c) \equiv C(a, c) \ | \ \forall a, b, c \in \mathcal{C} $$. In other words morphisms correspond to principle of compositionality.
Properties (or axioms):
  1. Associativity: $$ f_{a \to b}, g_{b \to c}, h_{c \to d} : \; h \circ (g \circ f) \equiv (h \circ g) \circ f $$.
  2. Identity morphisms (two-sided): $\forall x \, \exists  id_{x : x \to x},  \forall f_{a \to x},  \forall g_{x \to b} : \; id_x \circ f_{a \to x} \equiv f_{a \to x}, \; g_{x \to b} \circ id_x \equiv g_{x \to b}$ (both left and right identity)

...
=if= The type level information is complete
  ...
  =else=
    ...
    Infer the missing type level part from the term level. There are certain situations and structures where ambiguity arises and is unsolvable from the information of the term level.
...
=if= The kind level information is complete
...
Type level declaration is considered to "have a CUSK" is it has enough syntatic information to warrant completeness and start checking type level correspondence to term level, it is a ad-hock state ot type inferring.

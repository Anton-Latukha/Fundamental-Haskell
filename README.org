...
Additional to default settings it is useful to use =-W=, =-Wcompat=. =-Wall= is for purists and would raise noise. 
...
=-Werror= makes raises warnings into fatal errors, useful for CI.

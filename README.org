...
In Haskell there is only one category, Hask, so multifunctor in Haskell is still $$ (Hask \times Hask) \rightarrow Hask \Rightarrow Hask \rightarrow Hask $$endofunctor.

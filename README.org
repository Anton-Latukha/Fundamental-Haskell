...
- [[#good-code][Good code]]
  ...
  - [[#good-sometimes-the-proper-nature-algorithm-that-models-behaviour-of-many-objects-is-computation-heavy][<<<Good: Sometimes the proper nature algorithm that models behaviour of many objects is computation heavy>>>]]
- [[#bad-code][Bad code]]
...
:DRILL_EASE: 2.42
:DRILL_LAST_QUALITY: 4
:DRILL_LAST_REVIEWED: [2019-12-25 Wed 18:56]
:END:
...
:DRILL_LAST_REVIEWED: [2019-12-28 Sat 17:48]
:END:

/*/ - extending polymorphism to works with unlifted and lifted types.
...
:DRILL_EASE: 2.5
:DRILL_LAST_QUALITY: 2
:DRILL_LAST_REVIEWED: [2019-12-28 Sat 17:56]
:END:

It is property chosen for Haskell type system.
Haskell is based on Hindley-Milner type system, it is let-bound.
/*/ to have strict type inference - if =let= and =where= declarations are polymorphic - $\lambda$ declarations - should be not.

So this is illegal in Haskell:
#+begin_src haskell
foo :: (Int, Char)
foo = (\f -> (f 1, f 'a')) id
#+end_src
...
:DRILL_EASE: 2.5
:DRILL_LAST_QUALITY: 2
:DRILL_LAST_REVIEWED: [2019-12-28 Sat 17:58]
:END:
...
:DRILL_EASE: 2.36
:DRILL_LAST_QUALITY: 3
:DRILL_LAST_REVIEWED: [2019-12-28 Sat 16:23]
:END:
...
:DRILL_EASE: 2.5
:DRILL_LAST_QUALITY: 2
:DRILL_LAST_REVIEWED: [2019-12-28 Sat 18:01]
:END:
...
** <<<Good: Sometimes the proper nature algorithm that models behaviour of many objects is computation heavy>>> :drill:
:PROPERTIES:
...
Knowing math meaning love to apply it. But if we implement the "real" algorithms behind nature processes, we face the need to go through the computations of laws of all particles.

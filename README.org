...
Has Hom objects, gets away Hom sets, category is no longer small, now may be called large.
$$ hom(x,y) \in K $$.

Called: /*/ over =K=.
Hom objects are in monoidal category =K=, multiplication operation in which allows composition of Hom objects: 
...
Large and the equality definition (by using associativity and idintity "up to isomorphism" (and introducing associator, left/right unitor)).
...
Collection of morphisms is not nesessary a set, in practice - is.

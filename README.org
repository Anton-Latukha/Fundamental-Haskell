...
Saying: Haskell has immutable "variables" (term used in imperative programming sence). Read as: values (due to value semantics (see "Value")) are immutable, and Haskell expressions are functions (that are referentially transparent) are also, and since they binded to /*/, we say "/*"/ are immutable. (Except, when you hack Haskell with explicit funсtions.)

...
Due to full laziness deferring everything to the runtime execution - the lazy pattern is one-size-fits all, analogous to =_=, and so it does not produce any checks during compilation, and raises errors during runtime.

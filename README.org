...
(fmap id id) :: (a -> a)  -- fmap can map objects
(\ x -> fmap x id) :: (a -> b) -> (a -> b)
(flip fmap id) :: (a -> b) -> (a -> b) -- & functions just as well, polymorphism means functions are also objects
...
(fmap f) a = (f a) -- Remaps all source objects using a functor defined from power set functor by applying a the funciton

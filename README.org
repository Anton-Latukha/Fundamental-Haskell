...
|------------------+------------------------------------------------------------------------------+------------------------------------------------------------------------|
| $$ E $$          | $$ \begin{verbatim} <$> :: Functor f \Rightarrow (a → b) \to f a \to f b \end{verbatim} $$ | endofunctor (in Haskell power set functor used)                        |
| $$ \eta_{ID \to E} $$    | $$ \begin{verbatim} pure ∷ Applicative f \Rightarrow a \to f a \end{verbatim} $$         | unit (natural transformation for functors $$ ID \to \mathcal{P} $$)      |
| $$ \mu_{E \circ E \to E} $$ | $$ \begin{verbatim} join :: Monad f \Rightarrow f (f a) → f a \end{verbatim} $$        | join (natural transformation for functors $$ {\mathcal{P \circ P \to P}} $$) |
